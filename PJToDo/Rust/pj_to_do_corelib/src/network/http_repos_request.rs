...
impl PJHttpReposRequest {
    ...
    where F: FnOnce(Result<Repos, FetchError>) + std::marker::Sync + Send + 'static + std::clone::Clone {
        let repos_request_body = serde_json::to_string(&repos_request_body);

        match repos_request_body {
            Ok(repos_request_body_json) => {
                let repos_request_body_json = PJUtils::string_to_static_str(repos_request_body_json.to_string());
                let mut request = PJHttpRequest::request_with(PJRequestConfig::repos(), &repos_request_body_json);
            ...
            Err(e) => {
                pj_error!("parse repos_request_body: ReposRequestBody to json error: {}!!!", e);
                let err = network::http_request::FetchError::from(String::from(format!("parse repos_request_body: ReposRequestBody to json error: {}!!!", e)));
    ...
    where F: FnOnce(Result<Repos, FetchError>) + std::marker::Sync + Send + 'static + std::clone::Clone {
        if repos_url.is_empty() {
            ...
            let err = network::http_request::FetchError::from(String::from("repos_url can not be empty!!!"));
            completion_handler(Err(err));
        } else {
    ...
    where F: FnOnce(Result<Repos, FetchError>) + std::marker::Sync + Send + 'static + std::clone::Clone {
        if repos_url.is_empty() {
            ...
            *request.method_mut() = Method::DELETE;
            PJHttpReposRequest::do_repos_request(request, |result| {
            match result {
                Ok(_) => {
...
impl PJHttpReposRequest {
    ...
    where F: FnOnce(Result<ReposFile, FetchError>) + std::marker::Sync + Send + 'static + std::clone::Clone {
        PJHttpReposRequest::crud_file(create_file_request_body, FileActionType::Create, completion_handler);
    }

    pub fn update_file<F>(create_file_request_body: ReposFileBody, completion_handler: F) 
    where F: FnOnce(Result<ReposFile, FetchError>) + std::marker::Sync + Send + 'static + std::clone::Clone {
        PJHttpReposRequest::crud_file(create_file_request_body, FileActionType::Update, completion_handler);
    }
    ...
    pub fn crud_file<F>(file_request_body: ReposFileBody, file_action_type: FileActionType, completion_handler: F) 
    where F: FnOnce(Result<ReposFile, FetchError>) + std::marker::Sync + Send + 'static + std::clone::Clone {
        ...
        if !file_request_body.path.is_empty() {
            ...
            if file_action_type != FileActionType::Create {
                if file_request_body.sha.is_empty() {
                    ...
                    let err = network::http_request::FetchError::from(String::from("ReposFileBody sha can not be empty"));
                    completion_handler(Err(err));
                } else {
                    PJHttpReposRequest::crud_file_request(file_request_body, file_action_type, completion_handler);
                }
            } else {
                PJHttpReposRequest::crud_file_request(file_request_body, file_action_type, completion_handler);
            }
            ...
            let err = network::http_request::FetchError::from(String::from("ReposFileBody path can not be empty"));
            completion_handler(Err(err));
        }
    }

    pub fn crud_file_request<F>(file_request_body: ReposFileBody, file_action_type: FileActionType, completion_handler: F) 
    where F: FnOnce(Result<ReposFile, FetchError>) + std::marker::Sync + Send + 'static + std::clone::Clone {
        let update_file_api_url = format!("{}{}", PJRequestConfig::get_repos_contents_url(), file_request_body.path);

            let file_request_body = serde_json::to_string(&file_request_body);

            match file_request_body {
            Ok(file_request_body_json) => {
                let repos_request_body_json = PJUtils::string_to_static_str(file_request_body_json.to_string());
                let mut request = PJHttpRequest::request_with(&update_file_api_url, &repos_request_body_json);
            ...
            Err(e) => {
                pj_error!("file_request_body: ReposFileBody can not convert to json: {}", e);
                let err = network::http_request::FetchError::from(String::from(format!("file_request_body: ReposFileBody can not convert to json: {}", e)));
                completion_handler(Err(err));
            }
        }
    }
