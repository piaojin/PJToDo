...
public typealias PJResponseBlock = (_ data: Any?, _ isSuccess : Bool) -> Void
...
class PJHttpRequestConfig {
    ...
    public lazy var iDelegate: IPJToDoHttpRequestDelegate = {
        ...
        let requestResultBackBlock: (@convention(c) (UnsafeMutableRawPointer?, UnsafeMutableRawPointer?, Bool) -> Void)! = { (pointer, dataPointer, isSuccess) in
            if let tempPointer = pointer {
    ...
    //Rust回调Swift
    fileprivate func requestResult(data: UnsafeMutableRawPointer?, isSuccess: Bool) {
...
//Rust回调Swift
fileprivate func requestResult(user: UnsafeMutableRawPointer, data: UnsafeMutableRawPointer?, isSuccess: Bool) {
